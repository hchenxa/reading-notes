...
### ACME:把 03 章的人肉流程变成机器对话

03 章那张流程图是**人**在走;ACME 协议(RFC 8555)把它变成两台机器间的标准对话——[申请方] 的 ACME **客户端**(certbot、acme.sh…)对着 [CA] 的 ACME **服务器**(Let's Encrypt 的 Boulder、06 节的 step-ca 都实现它)。一次签发的完整对话:

```
[ACME 客户端(certbot)]                   [CA 的 ACME 服务器]
 1. 注册账号:生成账号密钥对 ────────→     从此每个请求都用账号私钥签名
 2. 下订单:要 example.com 的证书 ────→    + 一次性 nonce:证明"是你下的单,
 3.                      ←── 下发"作业"    且不是被抓包重放的"
                                       每个域名一道验证题(authorization +
                                       challenge = 03 章那三种:HTTP-01 /
                                       DNS-01 / TLS-ALPN-01)
 4. 做题:放文件 / 加 TXT 记录…  ────→    CA 亲自回来访问/查询 → 确认控制权
 5. 提交 CSR(域名密钥对自留) ────→       → 签名 → 下发证书链
```

和 03 章手动流程逐一对上:密钥对自留、CSR 自组、域名验证 = 三种 challenge 的自动化版、CA 签名下发——certbot 那几条命令,就是把上面 5 步一次性走完。真正多出来的只有 **account 机制**:先注册一对账号密钥,此后每个请求都用它签名,CA 才能确认"下订单的确实是同一个你"。

由此推出三件事,和前后章节都咬合:

- **续期 = 重新走一遍全流程,不是"续费"**:90 天到期前,客户端重新下订单、重新验证、重新拿证书——正因为全自动,证书短命才不疼,这就是 04 节"短命 + 自动化胜过完美吊销"的机制来源
- **协议把 client 与 CA 解耦**:标准定死后,任何客户端都能对接任何实现 ACME 的 CA——今天 Let's Encrypt,明天换成公司内部的 step-ca(06 节),certbot 改个服务器地址即可,工具链零学习成本
- **通配符只能走 DNS-01**:HTTP-01 要求有一台能被 CA 访问的主机放文件,`*.example.com` 背后是无数主机,没有这种落点;DNS-01 验证的是你能否改整个域的 DNS(03 节表格),天然覆盖通配——所以下面的清单里通配符要单独备 DNS 服务商 API
