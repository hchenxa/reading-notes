...
Informer 主要依靠 Watch 长连接来实时接收资源的增删改事件。但在复杂的网络环境中，Watch 连接可能会偶尔断开或丢包。

假设某个 Deployment 的副本数从 3 改为了 5，但这个“更新事件”恰好因为网络问题丢失了。如果没有 Resync，Controller 就永远不知道这个变化，导致实际状态（3个副本）与期望状态（5个副本）永远不一致。

有了 Resync，Informer 会周期性地触发 Sync 事件（在内部被当作 Updated 事件处理），强制让 Controller 重新审视一遍缓存中的对象，从而发现状态偏差并触发调谐（Reconcile）逻辑进行纠正。

### 🚀 零 API Server 负载（与轮询的本质区别）

- **纯轮询**：每隔一段时间直接向 API Server 发起 List 请求获取全量数据，这会给 API Server 带来持续的巨大压力。

- **Resync**：完全不访问 API Server。它只是把 Informer 本地缓存（Indexer）中已有的对象，重新生成事件放入 Delta FIFO 队列中，触发一次控制器的 OnUpdate 回调。

因此，Resync 是一种基于缓存的自省机制，它只是让控制器“复习”一遍本地已有的数据，对 API Server 是零负载的。

### 🛠️ 应对业务逻辑处理失败

除了防止 Watch 丢事件，Resync 还能应对 Controller 自身的业务 Bug。

比如，Controller 监听到了 Service 的创建事件，并调用外部 LB（负载均衡器）API 去创建对应的负载均衡。如果因为外部 LB 的 Bug 导致创建失败，但 Controller 的业务逻辑没有正确重试，状态就会不一致。

此时，Resync 机制会定期将这个 Service 对象重新推送到工作队列中，让 Controller 再次执行调谐逻辑，发现 LB 没创建成功并进行重试。
